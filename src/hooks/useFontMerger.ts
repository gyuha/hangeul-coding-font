  ...
  const mergefonts = useCallback(
    async (options: MergeOptions, fontName: string) => {
      ...
      try {
        ...
        try {
          const Font = (
            window as unknown as { opentype: { Font: new (options: unknown) => Font } }
          ).opentype.Font
          ...
          try {
            ...
            if (fontError instanceof Error && fontError.message.includes('lookup type')) {
              console.warn("OpenType lookup 오류로 인해 고급 기능 없이 폰트 생성:", fontError.message)
          ...
          // 글리프 상세 정보
          console.log('First 5 glyph details:', glyphsArray.slice(0, 5).map(g => ({
            name: g?.name || 'unnamed',
        ...
        try {
          ...
          try {
            ...
            if (serializationError instanceof Error && serializationError.message.includes('lookup type')) {
              console.warn("폰트 직렬화 중 lookup type 오류 발생, 단순 폰트로 재생성:", serializationError.message)
    ...
    (fontName: string) => {
      ...
      try {
        ...
        try {
          ...
          if (serializationError instanceof Error && serializationError.message.includes('lookup type')) {
            throw new Error("폰트에 지원되지 않는 고급 기능이 포함되어 있습니다. 다시 폰트를 합쳐주세요.")
          } else {
            throw serializationError
          }
        }
